                       ...
                       int numRows, int numCols)
{
  ...
  if( i < numRows*numCols){
    ...
    unsigned char rcolor = round(pow((px.x / 255.0f), (1.0f / 4.0f)  ) * 255.0f);
    greyImage[i].x = (rcolor > 255.0f) ? 255.0f : rcolor;
    greyImage[i].x = (rcolor < 0) ? 0 : rcolor;
    unsigned char gcolor = round(pow((px.y / 255.0f), (1.0f / 4.0f)  ) * 255.0f);
    greyImage[i].y = (gcolor > 255.0f) ? 255.0f : gcolor;
    greyImage[i].y = (gcolor < 0) ? 0 : gcolor;
    unsigned char bcolor = round(pow((px.z / 255.0f), (1.0f / 4.0f)  ) * 255.0f);
    greyImage[i].z = (bcolor > 255.0f) ? 255.0f : bcolor;
    greyImage[i].z = (bcolor < 0) ? 0 : bcolor;


    //printf( "Despues: R: %i G: %i B %i \n",greyImage[i].x,greyImage[i].y,greyImage[i].z );
  }
                  ...
                  uchar4* const d_greyImage, size_t numRows, size_t numCols)
{
  ...
  gamma_correction_kernel<<<gridSize, blockSize>>>(d_rgbaImage, d_greyImage, numRows, numCols);
